...
def search():
    if request.method == "GET":
        ...
        for i in range(len(directories)):
            ...
            if child_id is None:
                print("readPartition: cannot access '{}': No such file or directory".format(path))
                comb = {"command": "Failed: file not exists.", "result": result}
                return jsonify(comb=comb)
            else:
                child_id = child_id[0]
            parent_id = child_id
        ...
        for i in range(size):
            res = []
            data = []
            division_content = str(content[i])
            ...
            result.append({"partition " + str(i): res})

        result.append({"total": result2})
...
def analytics():
    if request.method == "GET":
        ...
        result = map_partition2(file, p)
        ans = reduce_partition(res, mode)
        result.append("Reduce part:")
        result.append({"Reduce:": str(ans)})
